...
export default function AboutSection() {
  ...
  const [counters, setCounters] = useState({ clients: 0, projects: 0, uptime: 0, support: 0 });

  const stats = [
    { icon: Users, number: 500, suffix: "+", label: "Happy Clients", key: "clients" },
    { icon: Award, number: 50, suffix: "+", label: "Projects Completed", key: "projects" },
    { icon: TrendingUp, number: 99.9, suffix: "%", label: "Uptime Guarantee", key: "uptime" },
    { icon: Target, number: 24, suffix: "/7", label: "Support", key: "support" }
  ];
      ...
      const animateCounter = (key, target, isFloat = false) => {
        ...
        const timer = setInterval(() => {
          ...
          if (current >= target) {
            ...
            setCounters(prev => ({
              ...prev,
              [key]: isFloat ? Math.round(current * 10) / 10 : Math.round(current)
            }));
          }
        }, duration / steps);
      };
  ...
  return (
    <section ref={ref} id="about" className="py-24 bg-gradient-to-b from-blue-50 to-white overflow-hidden relative">
          ...
          <motion.div {...depthFade} className="space-y-8">
            <div>
              ...
              <p className="text-lg text-gray-600 mt-4 leading-relaxed">
                Our mission is to bridge the gap between complex cloud technologies and business success.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              >
                <h3 className="font-bold text-gray-900 mb-2 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#10B5DB]" /> Our Vision
                </h3>
                <p className="text-sm text-gray-600">To democratize cloud technology and make it accessible to businesses worldwide.</p>
              </motion.div>
              ...
              >
                <h3 className="font-bold text-gray-900 mb-2 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#10B5DB]" /> Our Values
                </h3>
                <p className="text-sm text-gray-600">Innovation, reliability, security, and customer-centric solutions.</p>
              </motion.div>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {stats.map((stat, index) => (
              <motion.div
                ...
                className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-8 shadow-xl border border-blue-50 text-center relative group overflow-hidden"
              >
                {/* Subtle Hover Glow */}
