...
export default function LoginPage() {
  ...
  // get auth helpers/state
  const { login, isAuthenticated, loading, user } = useAuth();
  const router = useRouter();

  // Redirect only when auth check finished AND user is set
  useEffect(() => {
    if (!loading && isAuthenticated && user) {
      // Use replace so back button doesn't go to login
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // IMPORTANT: Do NOT router.push here. We rely on the effect above
      if (!result.success) {
        setError(result.message || "Incorrect email or password");
      }
      // If success, effect will redirect when auth state is ready
    } catch (err) {
