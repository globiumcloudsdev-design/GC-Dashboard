...
//src/app/api/attendance/frontend-auto-checkout/route.js
...
export async function POST(request) {
  try {
    ...
    if (attendanceId) {
      ...
      // Case B: Find today's attendance automatically (robust: date | checkInTime | createdAt)
      ...
      const userFilter = (userType === "agent") ? { agent: userId } : { user: userId };

      console.log("📋 Trying query by date/checkInTime/createdAt...");
      attendance = await Attendance.findOne({ ...timeRangeQuery, ...userFilter })
        .populate("shift")
        .populate("agent")
        .populate("user");

      if (!attendance) {
        console.warn("⚠️ No attendance found using time-range query. Attempting fallback: most recent attendance for user.");
        // Debug: fetch the most recent attendance for this user (helpful for tracing)
        const recent = await Attendance.find(userFilter).sort({ createdAt: -1 }).limit(5).lean();
        console.log("🔎 Recent attendance docs (up to 5):", recent.map(r => ({
          id: r._id,
          date: r.date,
        ...
        // Fallback: take the latest attendance (if any) and use it if checkInTime or createdAt is within a relaxed tolerance (same day)
        const mostRecent = await Attendance.findOne(userFilter).sort({ createdAt: -1 })
          .populate("shift")
          .populate("agent")
          .populate("user");

        if (mostRecent) {
          // If this appears to be today's attendance (compare checkInTime/createdAt with todayStart/todayEnd)
          const cti = mostRecent.checkInTime ? new Date(mostRecent.checkInTime) : null;
          const ca = mostRecent.createdAt ? new Date(mostRecent.createdAt) : null;
          const inToday = (cti && cti >= todayStart && cti < todayEnd) || (ca && ca >= todayStart && ca < todayEnd);

          if (inToday) {
            console.log("✅ Using mostRecent attendance (within today's range):", mostRecent._id);
            attendance = mostRecent;
          } else {
            console.warn("❌ mostRecent attendance is not within today's range. Returning not-found.");
            // keep attendance null so existing error flow returns message
          }
        } else {
          console.warn("❌ No attendance records found for user as fallback.");
        }
      }

      if (!attendance) {
        console.error("❌ No attendance record found for today (after robust checks)");
        return NextResponse.json({
          success: false,
      ...
      console.log("✅ Selected attendance:", attendance._id?.toString());
    }
