...
export async function GET(req, { params }) {
  try {
    ...
    const booking = await Booking.findById(id).populate("promoCodeId");
...
export async function PUT(req, { params }) {
  try {
    ...
    if (formData && (formData.date || formData.timeSlot)) {
      ...
      // Auto-set status to rescheduled if date/time changes
      if (formData.date !== originalBooking.formData.date || 
          formData.timeSlot !== originalBooking.formData.timeSlot) {
        updateData.status = "rescheduled";
        isRescheduled = true;
      }
    }
    ...
    // ✅ Email Notifications
    try {
      const { formData: bookingFormData, bookingId, webName, status: currentStatus } = updatedBooking;
      const websiteConfig = getWebsiteConfig(webName);
      const userEmail = bookingFormData.email;
      ...
      switch (currentStatus) {
        ...
        default:
          userSubject = `📋 Booking Status Updated: ${statusDisplay} - ${webName} (#${bookingId})`;
          userHtmlBody = getPendingEmail(updatedBooking, websiteConfig); // Fallback template
          break;
      }
...
export async function DELETE(req, { params }) {
  try {
    ...
    try {
      ...
      console.error('❌ Google Calendar event deletion failed:', calendarError.message);
    }
